...
public class Windows {
    ...
    public boolean doubleClickControl (String windowTitle, String windowText, WString id, WString button) {
        return mAutoIt.AU3_ControlClick (new WString (windowTitle), new WString (windowText), id, button, CLICK_TWICE, -1, -1) == 1;
    }

    public String getControlText (String windowTitle, String windowText, WString id) {
    ...
    public boolean sendTextToControl (String windowTitle, String windowText, WString id, String text, int mode) {
        return mAutoIt.AU3_ControlSend (new WString (windowTitle), new WString (windowText), id, new WString (text), mode) == 1;
    }

    public boolean setControlText (String windowTitle, String windowText, WString id, String text) {
        return mAutoIt.AU3_ControlSetText (new WString (windowTitle), new WString (windowText), id, new WString (text)) == 1;
    }

    public void sendText (String text, int mode) {
        mAutoIt.AU3_Send (new WString (text), mode);
    }

    public boolean run (String application, String workingDirectory, int mode) {
        int pid = mAutoIt.AU3_Run (new WString (application), new WString (workingDirectory), mode);
        return pid != 0;
    }

    public boolean closeWindow (String windowTitle, String windowText) {
        return mAutoIt.AU3_WinClose (new WString (windowTitle), new WString (windowText)) == 1;
    }

    public int waitForWindow (String windowTitle, String windowText, int timeout) {
        return mAutoIt.AU3_WinWait (new WString (windowTitle), new WString (windowText), timeout);
    }

    public boolean waitForWindowActive (String windowTitle, String windowText, int timeout) {
        return mAutoIt.AU3_WinWaitActive (new WString (windowTitle), new WString (windowText), timeout * MILLIS_PER_SECOND) != 0;
    }

    public boolean waitForWindowToClose (String windowTitle, String windowText, int timeout) {
